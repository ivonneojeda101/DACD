...
import static org.java.controllers.AbstractDataHandler.prepareGson;
...
public class MemoryDataManagement implements DataManagement {
	...
	public void storeData(String jsonData) throws BussinessUnitException {
		try {
			...
			if (handler != null) handler.handleData(jsonData, keyDate, this);
		}
		catch (Exception e) {
	...
	public void deleteData(List<String> keyDates) {
		...
		while (iterator.hasNext()) {
			...
			if (!mapDates.containsKey(key.split("-")[0])) iterator.remove();
		}
	}
